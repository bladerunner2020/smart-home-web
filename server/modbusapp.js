 ...
 */
var debug = require('./mydebug')('modbusapp');
var error = debug;
var log = debug;
...
var mb_vars = {
    ...
    var12: false,    // Коридор
    var13: false,    // Аквариум
    var14: false,
    var15: false     // Умная розетка
};
...
exports.readActuators = function(vars,sflags){
    readZwaveActuator(vars, sflags,'var15', 9);
...
read_modbus = function(sflags){
    ...
    client.readCoils (0, 8, function (resp, err) {
        ...
        read_modbus_element(sflags, 'var12', resp.coils[5]); // Коридор
    });
    ...
    client.readCoils (16, 8, function (resp, err) {
        ...
        read_modbus_element(sflags, 'var15', resp.coils[6]); // Умная розетка 2
    });
...
exports.syncronize2 = function(vars, sflags){
    if (!isFlag2(sflags)) {
        ...
        sync_element(vars, sflags, 'var14', -1); // Датчик замка двери

        sync_element(vars, sflags, 'var15', 13); // Умная розетка. -1 - означает, что не подключен к контроллеру
    }
};
...
function isFlag2(sflags){
    return (sflags['var1'] != SYNC_NONE) || (sflags['var2'] != SYNC_NONE) || (sflags['var3'] != SYNC_NONE) ||
        ...
        (sflags['var13']!= SYNC_NONE) || (sflags['var15'] != SYNC_NONE);
}
